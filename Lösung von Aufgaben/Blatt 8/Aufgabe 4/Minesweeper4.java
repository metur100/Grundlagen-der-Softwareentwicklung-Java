/******************************************************************************
 *  Compilation:  javac Minesweeper4.java
 *  Execution:    java Minesweeper4
 *
 *  Eingabe: Breite Hoehe Anzahl-an-Minen
 *  Ausgabe: Graphische Ausgabe des Boards und Audio-Wiedergabe, spielfaehig
 *
 * Verwenden Sie die Musterloesung von Minesweeper3 oder Ihre eigene
 * Implementierung und schreiben Sie eine Methode uncoverNeighbors, die alle
 * Nachbarfelder aufdeckt. Diese soll aufgerufen werden, wenn ein Feld aufdeckt
 * wurde, das keine Minen in der Nachbarschaft hat. Außerdem sollen rekursiv
 * alle Nachbarfelder eines Nachbarn aufgedeckt werden, falls der Nachbar
 * ebenfalls keine Mine in der Nachbarschaft hat. Geben Sie zudem zurueck, wie
 * viele Felder aufgedeckt wurden.
 *
 ******************************************************************************/
...
public class Minesweeper4 {
  ...
  /**
   * Deckt das uebergebene Feld und alle Nachbarn auf.
   * Deckt eventuell auch die Nachbarn eines Nachbarn usw. auf, wenn diese ebenfalls keine Mine in der Nachbarschaft haben.
   * Darf nur fuer ein Feld aufgerufen werden, dass keine Mine in der Nachbarschaft hat.
   * Gibt die Anzahl an aufgedeckten Feldern zurueck.
   */
  public static int uncoverNeighbors(int[][] board, int width, int height, int[][] uncovered) {
    // Zum Merken der aufgedeckten Felder
    ...
    // Keine Minen in der Nachbarschaft -> schreibe 0 ins Feld
    StdDraw.text(width + 0.5, height + 0.5, "0");
    ...
    for (int i = height - 1; i <= height + 1; i++) {
      ...
      for (int j = width - 1; j <= width + 1; j++) {
        if (i < maxHeight && i >= 0 && j < maxWidth && j >= 0 && uncovered[i][j] == 0) {
          int countedMines = countMines(board, j, i);
          if (countedMines == 0) {
            uncoveredFields += uncoverNeighbors(board, j, i, uncovered);
          } else {
            StdDraw.text(j + 0.5, i + 0.5, Integer.toString(countedMines));
            uncovered[i][j] = 1;
            uncoveredFields++;
          }
        }
  ...
  public static void main(String[] args) {
    ...
    //StdRandom.setSeed(System.currentTimeMillis()); // Einkommentieren fuer variierende Felder bei gleichen Parametern
	...
	// Pruefe, ob Feld bereits aufgedeckt wurde
	  ...
	  uncovered[height][width] = 1;
          ...
          if (!isMine(board, width, height)) {
            ...
            int countedMines = countMines(board, width, height);
            if (countedMines == 0) {
              // Decke aktuelles Feld und alle Nachbarn auf (eventl. rekursiv alle Nachbarn der Nachbarn)
              uncoveredFields += uncoverNeighbors(board, width, height, uncovered);
            } else {
	      // Decke nur aktuelles Feld auf
	      String text = Integer.toString(countedMines);
